    ...
    <div class="sider">
        <ul class="menus">
            <li class="menu-item" v-for="(menu, index) in menus" :key="index">
                {{ (((submenus = menu.items || []), void 0), ((route = menu.route ? '#' + menu.route : 'javascript: void(0)'), void 0)) }}
                <a class="menu-link clearfix" :class="{ active: path.indexOf(menu.path) > -1 }" :href="route" @click="toggleMenu(menu)">
                    <i :class="['icon', `icon-${menu.icon}`, { active: path.indexOf(menu.path) > -1 }]"></i>
                    <span>{{ menu.name }}</span>
