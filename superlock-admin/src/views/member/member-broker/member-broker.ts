...
export default class MemberBroker extends Vue {
    ...
    @memberModule.State('brokerForm') brokerForm!: BrokerForm;
    @memberModule.State('rateForm') rateForm!: RateForm;
    @memberModule.State('quotaForm') quotaForm!: QuotaForm;

    @memberModule.Mutation(TYPES.SET_STATES) setStates!: (payload: any) => any;
    @memberModule.Mutation(TYPES.CLEAR_STATES) clearStates!: () => any;

    @memberModule.Action('fetchPageBrokers') fetchPageBrokers!: () => any;
    ...
    // 处理券商模态框submit事件
    async handleBrokerSubmit(brokerForm: BrokerForm) {
        await this._submitBroker(brokerForm, false);
    }

    // 私有函数：提交利率信息
    async _submitRate(rateForm: RateForm, isCode: boolean) {
        try {
    ...
    // 处理利率商模态框submit事件
    async handleRateSubmit(rateForm: RateForm) {
        await this._submitRate(rateForm, false);
    }

    // 私有函数：提交额度信息
    async _submitQuota(quotaForm: QuotaForm, isCode: boolean) {
        try {
    ...
    // 处理额度商模态框submit事件
    async handleQuotaSubmit(quotaForm: QuotaForm) {
