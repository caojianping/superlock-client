...
export default {
    ...
    actions: {
        // 获取小额免审信息
        async fetchFreeTrial(
            context: IActionContext<IRiskState>
        ): Promise<void> {
        ...
        async setFreeTrial(
            ...
            let { type, freeTrial } = context.state,
                value = [
                    freeTrial.withdrawLimit,
                    freeTrial.everyrewardLimit,
                    freeTrial.promotionrewardLimit,
                    freeTrial.lockAmount
                ][type - 1];
            return await riskService.setFreeTrial(
                type,
                value,
                isCode,
                freeTrial.code
