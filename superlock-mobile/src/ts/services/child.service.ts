...
export class ChildService {
    // 校验下级利率表单
    public static validateChildRateForms(
        childUid: string,
        ...
        validator.addRule(
            key,
            { name: 'childUid', value: childUid },
            { required: true },
            { required: '下级成员UID不可以为空' }
        );
    ...
    public async fetchLockPromoteRates(): Promise<Array<LockPromoteRateModel>> {
        let result = await Caxios.get<Array<LockPromoteRateModel> | null>(
            { url: Urls.child.lockPromoteRates },
            CaxiosType.Token
        );
        return result || [];
    }
    ...
    // 获取下级分页列表
    public async fetchChilds(
        pageNum: number = 1,
        pageSize: number = 10
    ): Promise<Array<ChildModel>> {
    ...
    // 设置下级利率
    public async setChildRates(
        childUid: string,
        childRateForms: Array<ChildRateFormModel>
    ): Promise<boolean> {
        let result: ValidationResult = ChildService.validateChildRateForms(
            childUid,
            childRateForms
        );
        if (!result.status)
