...
export class LockResultModel {
    ...
    public status!: number; // 状态；0：订单创建；10：订单处理中； 20：计息中 ；30：锁仓到期；40：锁仓失败
    public remark!: string; // 备注
    public dcDailyIncome!: number; // 每日收益dc，币种固定为DC
    public dcTotalIncome!: number; // 总收益dc，币种固定为DC
    public bcbTotalIncome!: number; // 总收益bcb，币种固定为BCB
}
