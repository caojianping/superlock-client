                ...
                <li>
                    <Field class="code" :value="userForm.smsCode" clearable placeholder="请输入验证码" @input="handleFieldInput('smsCode', $event)">
                        <template slot="button">
                            <SmsCode :area-code="userForm.areaCode" :mobile="userForm.mobile" is-init @stop="handleSmsCodeStop" />
                        </template>
                    </Field>
                </li>
                <li>
                    <Button class="effect-shadow" type="primary" block round @click="nextStep">下一步</Button>
                </li>
            </ul>
        </div>

        <ForgetForm v-model="isShow" @submit="handleForgetFormSubmit" @close="handleForgetFormClose" />
