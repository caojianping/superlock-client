        ...
        <ul class="rate-forms">
            <li v-for="(defaultRateForm, index) in defaultRateForms" :key="index">
                <Field
                    ...
                    :label="`${defaultRateForm.length}${unitTypes[defaultRateForm.unit - 1]}锁仓利率(<=${defaultRateForm.max}%)`"
                >
                    <template slot="input">
                        <div class="scb-input">
                ...
                <Field v-if="defaultRateForm.type === 2" :label="`推广解锁利率(<=${defaultRateForm.max}%)`">
                    <template slot="input">
                        <div class="scb-input">
                            <input type="text" v-model="defaultRateForm.value" :placeholder="`<=${defaultRateForm.max}%`" />
                            <span>%</span>
            ...
            <li v-if="defaultRateStats && !defaultRateStats.existDefault" class="rate-prompt">
